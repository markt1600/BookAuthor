...
// Replace the whole manuscript with a freely-edited full text. The result is a
// single neutral passage (rendered without user/AI attribution) so a global edit
// reads as one clean voice in either mode. Author is "claude" so it remains the
// user's move next in participate mode; the `merged` flag drives neutral rendering.
export function mergeFullText(book, fullText) {
  ...
  const turns = text
    ...
    : [];
  const chapters = normalizeChapters(
    (book.chapters || []).filter((c) => Number(c.startTurn) === 0),
    turns.length
  );
  return { ...book, turns, chapters, updatedAt: Date.now() };
}
