 ...
 */
export async function guideStory({ title, author, guide, prompt, opening, recent, memory, whole, arc, sections, targetWords, bible, voiceSample, position, onDelta }) {
  const g = guide || {};
...
function craftNotes(m) {
  ...
  if (m.motifs) {
    lines.push(
      `Distinctive imagery, metaphors & phrasing already used in recent sections — do NOT reuse or echo any of these:\n${m.motifs}`
    );
  }
  return lines.join("\n\n");
}
 ...
 */
export async function continueStory({ title, author, settings, opening, recent, memory, whole, arc, sections, targetWords, bible, voiceSample, position, onDelta }) {
 ...
 */
export async function selfEditSection({ title, mode, guide, draft, memory, onDelta }) {
...
export async function analyzeStory({ title, fullText, prior, guide, eroticaLean, arc, sections, final }) {
  ...
  const system = [
    ...
    }${hasArc ? "arcProgress, arcDone, " : ""}${final ? "" : "motifs, "}continuity.`,
    guide
    ...
    final
      ? ""
      : '  "motifs": The most distinctive images, metaphors, sensory details, and pet phrases the manuscript has ALREADY used — especially anything used more than once, weighting the most recent sections heavily. 4-8 short fragments (each under ~8 words), newline-separated, no bullets or numbering. These are shown to the writer as a do-not-reuse list, so include only genuinely distinctive wording (never ordinary words or common actions).',
