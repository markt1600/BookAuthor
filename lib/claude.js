 ...
 */
export async function guideStory({ title, author, guide, prompt, opening, recent, memory, whole, arc, targetWords, onDelta }) {
  const g = guide || {};
...
// Format the long-horizon heading goals as soft, pace-aware steering.
function arcBlock(arc) {
  if (!Array.isArray(arc) || !arc.length) return "";
  const PACE = {
    eventually: "distant horizon — only the faintest, most organic drift toward this, if any",
    gradually: "gentle, natural progress — develop and foreshadow it, but do not resolve it yet",
    soon: "begin actively steering toward this now",
  };
  const lines = arc
    .filter((h) => h && h.text)
    .map((h) => `- ${h.text}  [${PACE[h.pace] || PACE.gradually}]`)
    .join("\n");
  if (!lines) return "";
  return (
    "Longer-term direction — where the book is heading. Work these in gradually across coming sections; " +
    "respect the pace noted on each, and do NOT force or fully resolve them in this one section unless it flows naturally:\n" +
    lines
  );
}
 ...
 */
export async function continueStory({ title, author, settings, opening, recent, memory, whole, arc, targetWords, onDelta }) {
...
export async function analyzeStory({ title, fullText, prior, guide, eroticaLean, arc }) {
  const hasArc = Array.isArray(arc) && arc.filter((h) => h && h.text).length > 0;
  const arcHeadings = hasArc
    ? arc
        .filter((h) => h && h.text)
        .map((h, i) => `${i + 1}. (pace: ${h.pace || "gradually"}) ${h.text}`)
        .join("\n")
    : "";
  const arcProgressLine =
    '  "arcProgress": Assess progress toward the author\'s heading goals listed below. Output EXACTLY one line per goal, IN THE SAME ORDER, newline-separated, with NO numbering or bullets. Each line is a brief, honest note on how far the story has actually moved toward that goal (e.g. "Untouched so far.", "Being set up; not yet surfaced.", "Actively developing.", "Essentially achieved."). Judge only what the manuscript shows.';
  ...
  const system = [
    ...
    `Use exactly these keys, IN THIS ORDER (all strings except qualityScore): style, genre, synopsis, quality, qualityScore, critique, nextDirection, suggestions, ${hasArc ? "arcProgress, " : ""}continuity.`,
    guide
  ...
  const suggestions = getStr("suggestions").trim();
  ...
  return {
    ...
    suggestions: suggestions || (prior && prior.suggestions) || "",
