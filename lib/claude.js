...
export async function reviseManuscript({ title, author, guide, mode, fullText, critique, quality, score, onDelta }) {
  ...
  const system = [
    ...
    "",
    "Your task:",
    "- Rewrite the ENTIRE manuscript from start to finish, preserving its core premise, characters, and essential plot, but materially strengthening it.",
    ...
    "- You may restructure scenes, add or remove material, merge or split passages, and rework any line — whatever genuinely serves the story.",
    "- Keep the chapter structure: emit a line `## Chapter` (or `## Chapter: Title`) at the start of each chapter, then its prose. Keep roughly the same chapter count unless merging or splitting clearly helps.",
    voice,
...
export async function revisionPlan({ title, fullText, critique, quality, score }) {
  const system = [
    "You are a master editor. Read the complete manuscript and the critique, then produce a concrete REVISION PLAN to raise the STORY to at least 90/100 by GENUINELY fixing its weaknesses — never by padding, inflating, or gaming a score.",
    "Output a tight numbered list (6-12 items) of specific, actionable changes grounded in THIS story: structural fixes, character and motivation depth, stakes, pacing, threads to resolve, and how to land the ending. No preamble, no commentary — just the numbered list.",
  ].join("\n");
  const user = [
    `Title: "${title}"`,
    ...
    .join("\n");
  const text = await runText(
    tuned({ model: MODEL, max_tokens: 1500, system, messages: [{ role: "user", content: user }] }, 0.5)
  );
  return (text || "").trim();
}
...
export async function reviseChunk({
  ...
  const system = [
    "You are a master editor rewriting a COMPLETED manuscript in consecutive passes to raise it to at least 90/100 — GENUINELY fixing weaknesses, never padding, inflating, or gaming a score.",
    "You are rewriting ONE consecutive part of the book. Follow the shared revision plan and keep continuity with the part already rewritten and with the overall story.",
    isFirst
      ? "- This is the FIRST part — open the book with confidence."
      : "- Continue SEAMLESSLY from the end of the already-rewritten text shown below. Do NOT recap, summarize, or repeat any of it.",
    "- Rewrite the SOURCE part below, implementing the plan where it applies: deepen characters and motivation, sharpen stakes, fix pacing, resolve threads, strengthen every beat.",
    "- Preserve `## Chapter` markers: emit a `## Chapter` (or `## Chapter: Title`) line exactly where a chapter begins in the source part.",
    isLast ? "- This is the FINAL part — bring the story to a satisfying, earned ending." : "",
    voice,
  ...
  const text = await runText(
    tuned(
      { model: MODEL, max_tokens: 12000, system, messages: [{ role: "user", content: user }] },
      0.9
    ),
    onDelta
  );
