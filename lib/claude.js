...
export async function continueStory({ title, author, settings, opening, recent, memory, whole, targetWords, onDelta }) {
  ...
  if (whole) {
    ...
    const tail = `Now write the next ~${target} words, continuing seamlessly.`;
...
export async function analyzeStory({ title, fullText, prior, guide, eroticaLean }) {
  ...
  const system = [
    ...
    "Use exactly these keys, IN THIS ORDER (all strings except qualityScore): style, genre, synopsis, quality, qualityScore, critique, nextDirection, suggestions, continuity.",
    guide
