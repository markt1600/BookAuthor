...
function maturityText(g) {
  ...
  return [
    ...
    "Stay within these intensities, keep it in service of the story, and follow content guidelines.",
  ].join(" ");
}
 ...
 */
export async function analyzeStory({ title, fullText, prior, guide }) {
  ...
  const system = [
    ...
    qualityLine,
    scoreLine,
    '  "nextDirection": ONE vivid sentence the author could use as the instruction for the next section — phrased as a direction (what should happen next), not a summary. Build naturally on where the story just left off.',
  ...
  let nextDirection = getStr("nextDirection").trim();
  if (nextDirection.length > 300) nextDirection = nextDirection.slice(0, 300).trim();
