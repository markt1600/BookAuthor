 ...
 */
export async function guideStory({ title, author, guide, prompt, opening, recent, memory, whole, targetWords, onDelta }) {
  const g = guide || {};
 ...
 */
export async function continueStory({ title, author, settings, opening, recent, memory, whole, targetWords, onDelta }) {
...
export async function analyzeStory({ title, fullText, prior, guide, eroticaLean }) {
  ...
  const critiqueLine = guide
    ? '  "critique":  2-3 short, specific points (newline-separated, no bullet characters) on the STORY\'s weakest aspects — what is holding it back and detracting from the score (e.g. thin stakes, an underdeveloped character, slack pacing, unclear motivation, a predictable turn). Be concrete and constructive. Do NOT critique the prose or writing craft.'
    : '  "critique":  2-3 short, specific points (newline-separated, no bullet characters) on the weakest aspects — what is holding the work back and detracting from the score. Be concrete and constructive.';
  const system = [
    ...
    "Use exactly these keys, IN THIS ORDER (all strings except qualityScore): style, genre, synopsis, quality, qualityScore, critique, nextDirection, continuity.",
    guide
  ...
  if (!style && !genre && !synopsis && !quality && !critique && score == null && !continuity && !nextDirection) {
    return null;
  }
